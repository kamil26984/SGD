...
def runGame(isFirstGame):
    ...
    while True: # main game loop
        drawBoard(mainBoard)
        displayCurrentPlayer(turn ,turnCounter)
        if turn == HUMAN:
            turnCounter += 1

            # Human player's turn.
            getHumanMove(mainBoard, showHelp, turn, turnCounter)
            if showHelp:
...
def getHumanMove(board, isFirstMove, turn, turnCounter):
    draggingToken = False
...
def displayCurrentPlayer(turn, turnCounter):
    currentPlayerText = f"Player: {turn.capitalize()} | Turn: {turnCounter}"
    font = pygame.font.Font(None, 36)
    textSurf = font.render(currentPlayerText, True, TEXTCOLOR)
